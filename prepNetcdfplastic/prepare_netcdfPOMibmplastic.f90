...
!!  This program reads INIT.DAT and writes netcdf plastic data
    ...
    parameter  (ip=0)
    parameter  (nadd=2*ip+1)                        !add more particles in the same grid box (e.g ip=1->np=9)
    parameter  (iskip=1)                            !particles in every iskip grid boxes
!    parameter  (nareas=n_upper$g*nadd*nadd/iskip/iskip-2)  !number of Initial area locations
    parameter (unitArea10=9.6450272E+01)                   !km2
    parameter (unitArea20=unitArea10/4.)
!    real weiarea(nareas)                           ! distribution weight for initial areas
!    real xilon(nareas),yilat(nareas)                !initial locations
    ...
    ref_id,iarea_id,fdep_id,icountdays_id,rbt_id,fou_id,nschoo_id
...
!---read INIT.DAT (grid+TSclim,Rmean,TS at boundaries)
    ...
    do 35 J=1,JMM1
        do 35 I=1,IM
            if(FSM(I,J) == 0.E0 .AND. FSM(I,J+1) /= 0.E0) DVM(I,J+1)=0.E0
    35 enddo
    do 40 J=1,JM
        do 40 I=1,IMM1
            if(FSM(I,J) == 0.E0 .AND. FSM(I+1,J) /= 0.E0) DUM(I+1,J)=0.E0
    40 enddo
!    call PRXY('ALON',TIME,alon,im,1,jm,1,1.E-1)
!    call PRXY('ALAT',TIME,alat,im,1,jm,1,1.E-1)
...
!    write(*,*)iw/25433
!    stop
    write(*,*)'initfish0',nareas,xilon(1),yilat(1)

    call initfish(nsi_local,nareas,init_nsi)
...
subroutine initfish(nsi_local,nareas,init_nsi)
