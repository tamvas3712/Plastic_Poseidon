...
!       include 'grid.h'
    ...
    parameter (nobsmicro=873,nobsmacro=430,nobsmicroSize=39)
...
!--model values at data points
    real :: conc_model_macro(6),conc_model_micro(6)
...
!      stop
    ...
    write(*,*)'ZZ',zz(kb-1)
    directory='../HindPlastic31/FISHOUT/'

    icount=0

!--Assim110scatter, Assim110scatter,
!--open
!      open(133,file='MacroValida_2010_2012med10newFreeContYiaNew.dat')
!      open(144,file='MicroValida_2010_2012med10newFreeContYiaNew.dat')
    open(155,file='MicroValidaSize_2010_2012med10newFree.dat')

!      open(123,file='ibm2010_2012med10newAssim110scatter.dat')
!      open(144,file='popdepav_microSize.dat')

!      open(12,file='popAll2010_2012annDep10med10FreeCont.bin',
!     & form='unformatted')
!      open(11,file='popAll2010_2012annDep1med10FreeCont.bin',
!     & form='unformatted')

!       open(12,file='popAll2010_2012annDep10med10Free.bin',
!     &form='unformatted')
!       open(12,file='popAll2010_2012monDep10med10Assim110scatter.bin',
!     &form='unformatted')
     


!       open(13,file='depAll2010seas.bin',
!     &form='unformatted')
!       open(15,file='popAll2010seasBot.bin',
!     &form='unformatted')
!      open(20,file='popAll2010seasDep10biom.bin'
!     &form='unformatted')
!      open(19,file='daysAll2010seasDep10.bin',
!     &form='unformatted')
!      open(22,file='distAll2010seasDep10.bin',
!     &form='unformatted')
!       open(14,file='popAll2010_2012monBea.bin',
!     &form='unformatted')
!       open(21,file='pop3dAll2010_2012mon.bin',
!     &form='unformatted')
!       open(16,file='denAll2010seasDep10.bin'
!     &form='unformatted')
!       open(17,file='sinAll2010seasDep10.bin'
!     &form='unformatted')
!       open(18,file='fouAll2010seasDep10.bin',
!     &form='unformatted')
...
!      nsum=90
    ...
    !      close(99)
        do nn=1,nmax
            read(99,end=1500)n
            read(99)species(n),xfi(n),nfi(n),variant(n),xsi(n),ysi(n),ref(n),fdep(n), &
        ...
        ! concentration (#/Km2)
            ...
            !--3d conc
                if(nsp == 1 .AND. iref == 0)then
                    do k=2,klev
                        ddpt=dpt(k)-dpt(k-1)
                        if(-fdep(ch) >= dpt(k-1) .AND. -fdep(ch) <= dpt(k)) then
                        !--convert to #/m3
    ...
    !     &  1.e-6*tbvariantBot(1,1),1.e-6*tbvariantBot(2,1),1.e-6*tbvariantBot(4,2),
        ...
        !       do ivariant=1,4
            do k=1,klev-1
                write(21)(conc3d(i,1,k,4),i=1,im2*jm2)
        ...
        !--Model validation
            ...
            open(1,file='macroAllyiannisNewMed10excCurmi.dat')
            open(2,file='microAllnew300yiannisNewMed10.dat')
            ...
            open(3,file='cozar_data_sizeDistribution.dat')

            do isize=1,6
                conc_model_micro(isize)=0.
                conc_model_macro(isize)=0.
            enddo

            do n=1,nobsmicroSize
                read(3,*)iiyear,iimo,iiday,xlat,xlon
                write(*,*)n,xlat,xlon
                ii=nint((xlon-xleft)/xs)+1
                jj=nint((xlat-ybot)/xs)+1
                iseas=(iimo-1)/3+1
            !         iseas=3
                ...
                write(155,117)iseas,conc_model_micro(3),conc_model_micro(4), &
                conc_model_micro(5),conc_model_micro(6),conc_model_macro(1), &
                conc_model_macro(2),conc_model_macro(3),conc_model_macro(4), &
                conc_model_macro(5)

                117 format(I2,1X,9(f9.2,1X))
            enddo
...
! Calculation of empty SIs,then fill with new eggs in ersem_fishGrad.F
    ...
    !       if(species(n).eq.2.and.variant(n).lt.3)factor=5.
        ...
        if(variant(n) == 0)then
            ...
            !                  if(isnan(fdep(n)))write(*,*)'TESTBAG',fou(n),ref(n),
            !     & nfi(n)
